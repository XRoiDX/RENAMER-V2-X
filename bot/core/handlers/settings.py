...
async def show_settings(m: "types.Message"):
    ...
    buttons_markup = [
        [types.InlineKeyboardButton(f"Uᴘʟᴏᴀᴅ As Dᴏᴄ  {'✅' if upload_as_doc else '❌'}",
                                    callback_data="triggerUploadMode")],
        [types.InlineKeyboardButton(f"Aᴘᴘʟʏ Cᴀᴘᴛɪᴏɴ {'✅' if apply_caption else '❌'}",
                                    callback_data="triggerApplyCaption")],
        [types.InlineKeyboardButton(f"Aᴘᴘʟʏ Dᴇꜰᴀᴜʟᴛ Cᴀᴘᴛɪᴏɴ {'❌' if caption else '✅'}",
                                    callback_data="triggerApplyDefaultCaption")],
        [types.InlineKeyboardButton("Sᴇᴛ Cᴜsᴛᴏᴍ Cᴀᴘᴛɪᴏɴ",
                                    callback_data="setCustomCaption")],
        [types.InlineKeyboardButton(f"{'Change' if thumbnail else 'Set'} Thumbnail",
                                    callback_data="setThumbnail")]
    ]
    if thumbnail:
        buttons_markup.append([types.InlineKeyboardButton("𝘚𝘩𝘰𝘸 𝘛𝘩𝘶𝘮𝘣𝘯𝘢𝘪𝘭👀",
                                                          callback_data="showThumbnail")])
    if caption:
        buttons_markup.append([types.InlineKeyboardButton("𝘚𝘩𝘰𝘸 𝘊𝘢𝘱𝘵𝘪𝘰𝘯👁️‍🗨️",
                                                          callback_data="showCaption")])
    buttons_markup.append([types.InlineKeyboardButton("𝘊𝘭𝘰𝘴𝘦 𝘔𝘦𝘴𝘴𝘢𝘨𝘦",
