...
async def rename_handler(c: Client, m: Message):
    ...
    if m.from_user.id not in Config.PRO_USERS:
        ...
        if is_in_gap:
            await m.reply_text("Sᴏʀʀʏ Sɪʀ,\n"
                               "Nᴏ Fʟᴏᴏᴅɪɴɢ Aʟʟᴏᴡᴇᴅ!\n\n"
                               f"Sᴇɴᴅ Aꜰᴛᴇʀ  `{str(sleep_time)}s` !!",
                               quote=True)
            return
    await add_user_to_database(c, m)
    if (not m.reply_to_message) or (not m.reply_to_message.media) or (not get_file_attr(m.reply_to_message)):
        return await m.reply_text("Rᴇᴘʟʏ Tᴏ Aɴʏ Dᴏᴄᴜᴍᴇɴᴛ/ᴠɪᴅᴇᴏ/ᴀᴜᴅɪᴏ Tᴏ Rᴇɴᴀᴍᴇ ɪᴛ!", quote=True)

    # Proceed
    editable = await m.reply_text("Nᴏᴡ Sᴇɴᴅ Mᴇ Nᴇᴡ Fɪʟᴇ Nᴀᴍᴇ!", quote=True)
    user_input_msg: Message = await c.listen(m.chat.id)
    if user_input_msg.text is None:
        await editable.edit("𝘗𝘳𝘰𝘤𝘦𝘴𝘴 𝘊𝘢𝘯𝘤𝘦𝘭✓")
    ...
    try:
        ...
        if not file_id:
            return await editable.edit("Fᴀɪʟᴇᴅ Tᴏ Rᴇɴᴀᴍᴇ\n\n"
                                       "Mᴀʏʙᴇ Yᴏᴜʀ Fɪʟᴇ ᴄᴏʀʀᴜᴘᴛᴇᴅ :(")
        await handle_big_rename(c, m, file_id, file_name, editable, file_type)
    except Exception as err:
        await editable.edit("Fᴀɪʟᴇᴅ Tᴏ Rᴇɴᴀᴍᴇ\n\n"
                            f"**Eʀʀᴏʀ:** `{err}`\n\n"
